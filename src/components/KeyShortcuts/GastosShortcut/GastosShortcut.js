...
function GastosShortcut({visible, onCancel}) {
    
    useHotkeys('shift+x', () => wichone2())
    useHotkeys('shift+z', () => wichone())
    useHotkeys('shift+g', () => openModal())
...
//http://localhost:4000/consortiums/1/units/1/outlays
    function postGastoConsorcio(event){
        ...
        "outlayPeriod": datePLiq.toISOString().split('T')[0],
...
function done(){
    ...
    function postGastoUnidad(event){
        ...
        "description": denominacion.current.value,
        "date": date.toISOString().split('T')[0],
        "outlayPeriod": datePGasto.toISOString().split('T')[0],
        "settledPeriod": datePLiq.toISOString().split('T')[0],
        "amount": importe.current.value,
        "voucherNumber": nrodecomp.current.value
        });
    ...
    const [estadoprueba, setestadoprueba] = useState(visible)
    const [estadoopen, setestadopen] = useState()
    function cambioestadoprueba(){
        setestadoprueba(prev => !prev)
        if(estadoprueba === false){
            setestadopen(false)
        }else{
            setestadopen(visible)
        }
    }
    return (
        <>
        ...
        <Box  sx={style}>
            ...
            <DatePicker dateFormat="yyyy/MM/dd" preventOpenOnFocus={true} selected={date} onChange={date => setDate(date)} />
            <p>Fecha Actual</p>
            </div>
            ...
            <DatePicker dateFormat="dd/MM/yyyy" preventOpenOnFocus={true} selected={date} onChange={date => setDate(date)} />
            <p>Fecha Actual</p>
            </div>
